            ...
            <div class="container">
                <div class="scrollable-container">
                    <q-form @submit.prevent="envioFormulario" @reset="borrar">
                        <h3>Actualizar dirección de envío </h3>
                        <!--CAMPO  DIRECCION-->
                        <q-input v-model="datosEnvio.direccion" label="Dirección" dense minlength="2" maxlength="100"
                            lazy-rules :rules="[
...
const { actualizarDireccionEnvio, usuario } = usuarioComposable();
...
const props = defineProps({
    formularioActualizarDireccionEnvio: Boolean,
})
...
const envioFormulario = async () => {
    try {
        const response = await actualizarDireccionEnvio(datosEnvio);
        console.log("response ", response);

        if (response.data === 'Dirección añadida y actualizada exitosamente en los pedidos') {
            emit('cerrarFormulario', true)
            mostrarAlertaExito('Dirección actualizada exitosamente', quasar);
            const direccionFormateada = formatearDireccion(datosEnvio);
            usuario.value.pedidos.direccionEnvio.push(direccionFormateada);
        }
        ...
        if (response.data === 'Solo puedes actualizar la dirección de pedidos con estado PENDIENTE_CONFIRMACION_DIRECCION') {
            mostrarAlertaError('Solo puedes actualizar la dirección de pedidos con estado PENDIENTE_CONFIRMACION_DIRECCION', quasar);
        }
